...
# - Define gridlines
xlocsnow = np.concatenate([np.linspace(120,180,3),
                           np.linspace(-180,-90,4)])
# needs 180,-180 repeated to make lat horiz lines go through
ylocsnow = np.linspace(-60, 40, 6)
ylocsnow = np.append(ylocsnow,90); ylocsnow = np.insert(ylocsnow,0,-90)
# needed to make lon vert lines go through

# - Draw gridlines
gl = ax.gridlines(
    crs=ccrs.PlateCarree(), draw_labels=False, # take care of labels below
    linewidth=0.5, color='gray', alpha=0.5, linestyle='--')
gl.xlocator = mticker.FixedLocator(xlocsnow)
gl.ylocator = mticker.FixedLocator(ylocsnow)

# - Add lat and lon labels
xticksnow = np.concatenate([np.linspace(120,180,3),
